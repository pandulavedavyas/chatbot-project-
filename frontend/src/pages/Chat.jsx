          ...
          {messages.length === 0 ? (
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 mt-6 max-w-md justify-center">
                {['Explain quantum computing', 'Write a Python function', 'Help me debug code'].map((prompt) => (
                  <button
                    key={prompt}
                    onClick={() => setInputValue(prompt)}
                    className="px-3 py-1.5 text-xs text-surface-400 bg-surface-800/50 border border-surface-700/50 rounded-lg hover:border-primary-500/30 hover:text-primary-300 transition-all"
                  >
                    {prompt}
                  </button>
                ))}
