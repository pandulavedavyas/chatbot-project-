...
const Sidebar = ({ isOpen, onClose, onSelectConversation }) => {
  const { conversations, loadConversations, deleteConversation, currentConversation } = useChatContext();
  const [isLoaded, setIsLoaded] = useState(false);

  useEffect(() => {
    loadConversations().then(() => setIsLoaded(true));
  }, [loadConversations]);
              ...
              {conversations.map((conversation) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-surface-500 mt-0.5">{formatDate(conversation.updated_at)}</p>
                  </div>
                  <button
                    onClick={(e) => handleDeleteConversation(e, conversation.id)}
                    className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100 hover:bg-surface-700/50 transition-all duration-150 flex-shrink-0"
